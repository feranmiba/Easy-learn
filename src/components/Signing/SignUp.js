  ...
  const handleSubmit = (e) => {
    try {
      ...
      Axios.post(`${api}/signup`, profile).then(res => {
       ...
       isLoading(true)
       if (res.data.success) {
        navigate("/profiles",{ state: {username: profile.username, email:profile.email}}) 
       }
    ...
    <p className='border px-10 py-5 bg-white rounded-lg'>{errMesssage}</p>
      ...
      <section className=' flex justify-center items-center text-lg py-10'>
        <form  onSubmit={handleSubmit}   className='w-[80%] px-6 md:px-10 py-3 md:w-[40%] rounded-xl shadow-2xl text-[#FAF2F2] bg-[#2E303E]' >
          ...
          <input type='text' placeholder='Create your username' className='w-[90%] p-3 mt-3 bg-transparent outline-none' name="username" value={profile.username} onChange={handleChange}/>
          </div>


          <div className='mt-5'>
          <p className='font-bold'>Email</p>
          <input type='email' placeholder='Input your E-mail' className='w-[90%] p-3 mt-3 bg-transparent outline-none' name="email" value={profile.email} onChange={handleChange}/>
          </div>

          <div className='mt-5 mb-3'>
          <p className='font-bold'>Password</p>
          <input type='password' placeholder='Create your password' className='w-[90%] p-3 mt-3 bg-transparent outline-none' name="password" value={profile.password} onChange={handleChange}/>
          </div>


          <div className='flex flex-col text-white items-center gap-3'>
          <button className=' bg-[#242832] rounded-xl p-3 w-[80%] font-bold' type='submit'>Sign up</button>    
